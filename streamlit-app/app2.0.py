"""
大学生财务管理系统 - Campus Finance Tracker v10.0 (云端永久存储版)
修复：登录/注册数据本地存储 → 云端数据库，永不丢失
运行：streamlit run finance_app.py
依赖：pip install streamlit plotly pandas openpyxl supabase
"""
...
def render_sidebar(settings):
    ...
    with st.sidebar:
        st.markdown("""<div style="text-align:center;padding:1.2rem 0 .8rem">
          ...
          <div style="font-size:.78rem;opacity:.55">v10.0 · 云端永久存储版</div>
        </div>""", unsafe_allow_html=True)
        ...
        st.markdown('<div style="text-align:center;font-size:.7rem;opacity:.35;padding:.8rem 0">密码 SHA-256 加密 · 云端永久存储<br>Made with ❤️ for Students</div>', unsafe_allow_html=True)
