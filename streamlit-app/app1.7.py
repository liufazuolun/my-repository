"""
大学生财务管理系统 - Campus Finance Tracker v9.1
修复：plotly 不支持8位十六进制颜色（#RRGGBBAA）问题，全部转为 rgba()
...
# ============================================================
# 颜色工具：将 #RRGGBB + 2位透明度后缀 转为 rgba()
# plotly 不接受8位十六进制，CSS 可以接受，所以只在传给 plotly 时转换
# ============================================================
def hex_alpha(hex6: str, alpha: float) -> str:
    """将 #RRGGBB 和 0-1 的透明度转为 rgba(r,g,b,a)"""
    h = hex6.lstrip("#")
...
# ============================================================
# 图表（关键修复：grid_color 使用 hex_alpha() 转为 rgba）
# ============================================================
def apply_layout(fig, t, title_text="", has_axes=False, grid_color=None):
    ...
    if has_axes and grid_color:
        # grid_color 已经是合法 rgba() 字符串，直接传入
...
def render_charts(df, theme_name):
    ...
    # ★ 修复核心：用 hex_alpha() 生成合法的 rgba()，而非 #RRGGBB + "33"
    ...
    with tab4:
        if df.empty:
            ...
            for tp, color in [("收入", t["income"]), ("支出", t["expense"])]:
                ...
                # fillcolor 也需要合法格式——直接用 hex_alpha
...
def render_sidebar(settings):
    ...
    with st.sidebar:
        st.markdown("""<div style="text-align:center;padding:1.2rem 0 .8rem">
          ...
          <div style="font-size:.78rem;opacity:.55">v9.1 · 多账号安全版</div>
        </div>""", unsafe_allow_html=True)
