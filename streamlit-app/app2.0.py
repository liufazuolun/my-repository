...
def load_auth() -> dict:
    try:
        res = supabase.table("users").select("username,password_hash,display_name,created_at").execute()
        users = {}
        for u in res.data:
            users[u["username"]] = u
        return users
...
def change_password(username: str, old_pw: str, new_pw: str) -> bool:
    if not verify_password(username, old_pw):
        return False
    supabase.table("users").update({"password_hash": hash_password(new_pw)}).eq("username", username).execute()
    return True
...
SUBCAT_TO_BIGCAT = {sub: big for big, subs in EXPENSE_STRUCTURE.items() for sub, info in subs.items()}
...
THEMES = {
    "极光紫": {"primary": "#7C3AED", "secondary": "#A78BFA", "accent": "#F59E0B", "bg": "#0F0A1E", "card": "#1E1535",
               "text": "#F3F0FF", "income": "#10B981", "expense": "#EF4444", "mode": "dark"},
    "珊瑚橙": {"primary": "#EA580C", "secondary": "#FB923C", "accent": "#0EA5E9", "bg": "#1C0F0A", "card": "#2D1A10",
               "text": "#FFF7F0", "income": "#22C55E", "expense": "#F43F5E", "mode": "dark"},
    "海洋蓝": {"primary": "#0369A1", "secondary": "#38BDF8", "accent": "#F97316", "bg": "#F0F9FF", "card": "#FFFFFF",
               "text": "#0C4A6E", "income": "#059669", "expense": "#DC2626", "mode": "light"},
    "抹茶绿": {"primary": "#15803D", "secondary": "#4ADE80", "accent": "#A21CAF", "bg": "#F0FDF4", "card": "#FFFFFF",
               "text": "#14532D", "income": "#16A34A", "expense": "#B91C1C", "mode": "light"},
    "樱花粉": {"primary": "#BE185D", "secondary": "#F472B6", "accent": "#7C3AED", "bg": "#FDF2F8", "card": "#FFFFFF",
               "text": "#831843", "income": "#0D9488", "expense": "#E11D48", "mode": "light"},
    "深夜黑": {"primary": "#E2E8F0", "secondary": "#94A3B8", "accent": "#F59E0B", "bg": "#0D1117", "card": "#161B22",
               "text": "#E6EDF3", "income": "#3FB950", "expense": "#F85149", "mode": "dark"},
}
...
# ============================================================
IRRATIONAL_RULES = [
    {"keywords": ["奶茶", "碳酸", "可乐", "饮料"], "apply_cats": ["街头小吃", "零食囤货", "其他支出", "宿舍自炊"],
     "icon": "🧋", "msg": "含糖饮料累计花费惊人，建议每周不超过2次，多喝白开水，既省钱又健康。"},
    {"keywords": ["外卖"], "apply_cats": ["正餐外卖", "夜宵外卖", "饮品外卖"], "amount_threshold": 35, "icon": "🛵",
     "msg": "外卖比食堂贵30%-50%，建议每周外卖不超过3次，能去食堂就去食堂！"},
    {"keywords": ["夜宵", "深夜"], "apply_cats": ["夜宵外卖", "街头小吃"], "icon": "🌙",
     "msg": "频繁吃夜宵影响睡眠和健康，建议保持规律作息，减少深夜进食。"},
    {"keywords": ["打车", "网约车", "滴滴", "出租"], "apply_cats": ["市内交通"], "amount_threshold": 20, "icon": "🚕",
     "msg": "打车比公交贵5-10倍！建议规划出行时间，多选公交、地铁或骑行。"},
    {"keywords": ["游戏", "充值", "内购", "氪金", "皮肤"], "apply_cats": ["游戏相关"], "icon": "🎮",
     "msg": "游戏充值极易失控！建议设定月度上限（不超过总支出5%），避免为虚拟道具大额消费。"},
    {"keywords": ["直播", "打赏", "礼物"], "apply_cats": ["直播打赏"], "icon": "📱",
     "msg": "直播打赏是高度冲动性消费！强烈建议关闭礼物功能，这类消费对你毫无实际价值。"},
    {"keywords": ["烟", "香烟"], "apply_cats": None, "icon": "🚬",
     "msg": "购买香烟既有害健康又浪费金钱，强烈建议戒烟或不要开始吸烟。"},
    {"keywords": ["白酒", "啤酒", "红酒"], "apply_cats": ["朋友小聚", "生日节日聚餐", "街头小吃", "其他支出"],
     "icon": "🍺", "msg": "频繁饮酒影响健康和学习状态，聚会适量即可，避免养成饮酒习惯。"},
    {"keywords": ["彩妆", "口红", "粉底", "眼影"], "apply_cats": ["护肤美妆"], "amount_threshold": 200, "icon": "💄",
     "msg": "大学阶段建议以基础护肤为主，避免跟风购买网红彩妆，超出实际需求。"},
    {"keywords": ["网红", "盲盒", "抽奖"], "apply_cats": ["盲盒抽奖", "其他支出"], "icon": "📦",
     "msg": "盲盒/网红商品营销溢价高，建议理性评估实际使用价值，避免冲动跟风。"},
    {"keywords": ["红包", "份子钱"], "apply_cats": ["人情红包"], "amount_threshold": 150, "icon": "🧧",
     "msg": "大额红包/份子钱容易形成攀比压力，心意比金额重要，学生阶段量力而行即可。"},
    {"keywords": ["会员", "VIP"], "apply_cats": ["视频会员", "音乐会员", "软件会员"], "icon": "🔑",
     "msg": "办会员前确认实际使用频率！多平台叠加是隐性开销，建议定期审查取消不常用订阅。"},
    {"keywords": ["演唱会", "应援", "专辑"], "apply_cats": ["追星周边"], "icon": "⭐",
     "msg": "追星消费容易失控，建议设定追星年度预算，爱豆不会因为你多买一张专辑而喜欢你。"},
    {"keywords": ["保健品", "蛋白粉", "代餐"], "apply_cats": ["保健品"], "icon": "💊",
...
def load_wallets(username=None):
    ...
    try:
        res = supabase.table("wallets").select("*").eq("username", username).execute()
        if res.data:
            return res.data[0]
    except:
        pass
...
def save_wallets(w, username=None):
    ...
    w["username"] = username
    try:
        supabase.table("wallets").upsert(w, on_conflict="username").execute()
    except:
        pass
...
def load_data(username=None):
    ...
    try:
        res = supabase.table("records").select("*").eq("username", username).execute()
...
def save_data(df, username=None):
    ...
    for r in records:
        supabase.table("records").upsert(r, on_conflict="id").execute()
...
def save_settings(s, username=None):
    if not username:
        return
    s["username"] = username
    supabase.table("settings").upsert(s, on_conflict="username").execute()
...
def get_period_df(df, period, cs=None, ce=None):
    ...
    if period == "今日":
        ...
        s = now - timedelta(days=150);
        e = now
    elif period == "全部":
        return df
    elif period == "自定义" and cs and ce:
        s = pd.Timestamp(cs);
        e = pd.Timestamp(ce) + timedelta(days=1)
    else:
        s = now.replace(day=1, hour=0, minute=0, second=0);
        e = now
            ...
            with st.form("login_form", clear_on_submit=False):
                ...
                if submitted:
                    if not username or not password:
                        ...
                        st.session_state.wallets_initialized = st.session_state.settings.get("wallets_initialized",
                                                                                             False)
...
def render_init_wizard():
    ...
    with mid:
        st.markdown('<div class="guide-tip">💡 如实填写当前各账户余额，填 0 表示该账户暂无余额或不使用。</div>',
                    unsafe_allow_html=True)
        init_vals = {}
        for name in WALLET_NAMES:
            icon = WALLET_ICONS.get(name, "💰")
            init_vals[name] = st.number_input(f"{icon} {name} 当前余额（元）", min_value=0.0, value=0.0, step=10.0,
                                              key=f"init_{name}")
...
def render_wallet_panel(theme_name):
    ...
    with st.expander("✏️ 修改账户余额", expanded=False):
        st.markdown('<div class="guide-tip">💡 充值/提现/转账等账外变动时，直接更新余额，系统自动记录差额。</div>',
                    unsafe_allow_html=True)
        edit_cols = st.columns(2)
        new_vals = {}
        for i, name in enumerate(WALLET_NAMES):
            with edit_cols[i % 2]:
                new_vals[name] = st.number_input(f"{WALLET_ICONS.get(name, '')} {name}（元）", min_value=0.0,
                                                 value=float(wallets.get(name, 0.0)), step=1.0,
...
def render_charts(df, theme_name):
    ...
    with tab1:
        if exp_df.empty:
            st.markdown('<div class="empty-state"><div class="empty-icon">📊</div><p>暂无支出数据</p></div>',
                        unsafe_allow_html=True)
        else:
    ...
    with tab3:
        if df.empty:
            st.markdown('<div class="empty-state"><div class="empty-icon">📊</div><p>暂无数据</p></div>',
                        unsafe_allow_html=True)
        else:
            dc = df.copy()
    ...
    with tab5:
        if exp_df.empty:
            st.markdown('<div class="empty-state"><div class="empty-icon">🗓️</div><p>暂无支出数据</p></div>',
                        unsafe_allow_html=True)
        else:
            dc = exp_df.copy()
    ...
    with tab6:
        if exp_df.empty:
            ...
            for bigcat, (lo, hi) in BIGCAT_RANGE.items():
                ...
                rows.append({"大类": bigcat, "实际占比": f"{ar:.1f}%", "建议区间": f"{lo}%～{hi}%", "状态": status,
                             "金额": fmt(actual)})
...
def get_record_advice(row, all_expense_df):
    ...
    return advice_map.get(cat,
                          f"💡 「{bigcat}·{cat}」{fmt(amount)}（P{priority} {PRIORITY_LABEL[priority]}）· 该类占总支出 {ratio:.1f}%。")
...
def render_advice(df, settings, theme_name):
    ...
    st.markdown('<div class="section-header"><span>🏦</span><span class="section-title">资产健康状态</span></div>',
                unsafe_allow_html=True)
    if total_assets < 200:
        st.markdown(
            f'<div class="advice-card urgent-card">🚨 <b>紧急预警</b>：总资产仅 {fmt(total_assets)}！建议立即联系家人补充生活费。</div>',
            unsafe_allow_html=True)
    elif total_assets < 500:
        st.markdown(
            f'<div class="advice-card warning-card">⚠️ 余额偏低：总资产 {fmt(total_assets)}，优先保障餐饮和交通。</div>',
            unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="advice-card good-card">✅ 资产良好：总资产 {fmt(total_assets)}，继续保持理性消费。</div>',
                    unsafe_allow_html=True)

    if not exp_df.empty:
        st.markdown(
            '<div class="section-header" style="margin-top:1.5rem"><span>💡</span><span class="section-title">本月预算监控</span></div>',
            unsafe_allow_html=True)
        total_exp = exp_df["amount"].sum()
        ...
        if ratio >= 100:
            st.error(f"⚠️ 已超预算 {fmt(total_exp - budget)}！")
        elif ratio >= 80:
            st.warning(f"⚡ 预算已用 {ratio:.1f}%，剩余 {fmt(budget - total_exp)}")
        else:
            st.success(f"✅ 预算良好！剩余 {fmt(budget - total_exp)}")

        st.markdown(
            '<div class="section-header" style="margin-top:1.5rem"><span>🚨</span><span class="section-title">不理性消费扫描</span></div>',
            unsafe_allow_html=True)
        found_any = False
        for _, row in exp_df.iterrows():
            ...
            if warnings_list:
                ...
                for w in warnings_list:
                    st.markdown(
                        f'<div class="advice-card irrational-card">{w}<br><span style="font-size:.78rem;opacity:.6">{row["category"]} · {fmt(row["amount"])} · {date_str}</span></div>',
                        unsafe_allow_html=True)
        if not found_any:
            st.markdown('<div class="advice-card good-card">🎉 未检测到明显不良消费行为，继续保持！</div>',
                        unsafe_allow_html=True)
...
def render_records_management(df, settings, theme_name):
    ...
    with tab1:
        st.markdown('<div class="guide-tip">💡 先选大类，再选具体子类，系统将给出精准消费建议。</div>',
                    unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
    ...
    with tab2:
        if df.empty:
            st.markdown('<div class="empty-state"><div class="empty-icon">📝</div><p>还没有任何记录</p></div>',
                        unsafe_allow_html=True)
            return
        ...
        if len(dr) == 2:
            filt = filt[
                (filt["date"] >= pd.Timestamp(dr[0])) & (filt["date"] <= pd.Timestamp(dr[1]) + timedelta(days=1))]

        sort_map = {"日期降序": ("date", False), "日期升序": ("date", True), "金额降序": ("amount", False),
                    "金额升序": ("amount", True)}
        ...
        if st.button("🗑️ 删除该记录", type="secondary"):
            try:
                ...
                if 0 <= idx < len(filt_list):
                    ...
                    if pm in wallets:
                        wallets[pm] = (wallets[pm] + row["amount"]) if row["type"] == "支出" else max(0.0,
                                                                                                      wallets[pm] - row[
                                                                                                          "amount"])
                        st.session_state.wallets = wallets
                        save_wallets(wallets, username)
                    supabase.table("records").delete().eq("id", row["id"]).execute()
                    st.session_state.df = load_data(username)
                    st.success("记录已删除");
                    st.rerun()
                else:
    ...
    with tab3:
        ...
        with c1:
            ...
            if up:
                try:
                    imp = pd.read_csv(up).rename(
                        columns={"类型": "type", "子类": "category", "金额(元)": "amount", "日期": "date",
                                 "备注": "note", "支付账户": "payment_method"})
        ...
        with c2:
            ...
            if not df.empty:
                ...
                exp = exp.rename(
                    columns={"type": "类型", "category": "子类", "amount": "金额(元)", "date": "日期", "note": "备注",
                             "payment_method": "支付账户"})
                exp = exp[["日期", "类型", "大类", "子类", "金额(元)", "支付账户", "备注"]]
                buf = io.StringIO()
                exp.to_csv(buf, index=False, encoding="utf-8-sig")
...
def render_sidebar(settings):
    ...
    with st.sidebar:
        st.markdown("""<div style="text-align:center;padding:1.2rem 0 .8rem">
          ...
          <div style="font-size:.78rem;opacity:.55">v10.1 · 稳定修复版</div>
        </div>""", unsafe_allow_html=True)
        ...
        st.markdown("**💰 月度预算（元）**")
        budget = st.number_input("", min_value=0.0, value=float(settings.get("monthly_budget", 2000.0)), step=100.0,
        ...
        with st.expander("🔑 修改密码", expanded=False):
            ...
            if st.button("更新密码", use_container_width=True):
                if not old_pw or not new_pw1:
                    ...
                    st.error("新密码至少6位")
                elif change_password(username, old_pw, new_pw1):
                    st.success("✅ 密码已更新！")
                else:
                    st.error("当前密码错误")
        ...
        if st.button("🚪 退出登录", use_container_width=True):
            for key in ["logged_in", "username", "display_name", "df", "wallets", "settings", "wallets_initialized",
                        "confirm_clear"]:
                st.session_state.pop(key, None)
            st.rerun()

        st.markdown(
            '<div style="text-align:center;font-size:.7rem;opacity:.35;padding:.8rem 0">密码 SHA-256 加密 · 云端永久存储<br>Made with ❤️ for Students</div>',
            unsafe_allow_html=True)
...
def main():
    ...
    total_assets = sum(st.session_state.wallets.values()) if st.session_state.wallets else 0
    display_name = st.session_state.display_name
    ...
    if sm.get("charts", True):
        with st.expander("📈 可视化图表", expanded=not df.empty):
            if df.empty:
                st.markdown(
                    '<div class="empty-state"><div class="empty-icon">📈</div><p>添加记录后图表将在此显示</p></div>',
                    unsafe_allow_html=True)
            else:
