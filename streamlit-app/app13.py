"""
大学生财务管理系统 - Campus Finance Tracker v6.0
...
# ============================================================
IRRATIONAL_KEYWORDS = {
    "奶茶": ("🧋", "警惕高频奶茶消费！每杯约15-35元，每周3杯月均花费200+元，且高糖高脂不利健康。建议每周最多1-2次，用自制饮品或白开水替代。"),
    "外卖": ("🛵", "频繁点外卖是典型的高消费行为！外卖均价高于食堂30%-50%，且餐盒污染环境。建议每周外卖不超过3次，多去食堂。"),
    "打车": ("🚕", "频繁打车会快速消耗预算！比公交贵5-10倍。建议规划好出行时间，优先选择公交、地铁或骑行。"),
    "游戏": ("🎮", "游戏充值/内购极易上瘾失控！建议设定月度上限，避免为虚拟道具大额消费，将时间和金钱投入现实成长。"),
    "直播": ("📱", "直播打赏是高度冲动性消费，极易超支！建议关闭直播间礼物功能，避免在情绪激动时充值。"),
    "烟":   ("🚬", "购买香烟既有害健康又浪费金钱，长期消费累积金额可观。强烈建议戒烟或不要开始吸烟。"),
    "酒":   ("🍺", "频繁购买酒类会影响健康和学习状态，聚会适量即可，避免养成饮酒习惯。"),
    "宵夜": ("🌙", "频繁消费宵夜既不健康又增加额外支出，影响睡眠质量。建议保持规律作息，减少深夜进食。"),
    "碳酸": ("🥤", "碳酸饮料高糖低营养，长期饮用损害健康。建议用白开水、茶水替代，既省钱又健康。"),
    "彩妆": ("💄", "注意彩妆消费是否超出实际需求！大学阶段建议以基础护肤为主，避免跟风购买网红产品。"),
    "网红": ("📦", "购买网红商品前请三思！网红营销溢价高，建议理性评估实际使用价值，避免冲动跟风。"),
    "盲盒": ("🎁", "盲盒消费具有赌博性质，容易上瘾！建议严格限制盲盒消费，将资金用于更有价值的支出。"),
    "红包": ("🧧", "大额红包/转账容易形成攀比压力，超出实际承受范围。心意比金额重要，建议量力而行。"),
    "会员": ("🔑", "办理会员前请确认实际使用频率！多个平台会员叠加是隐性开销。建议定期审查，取消不常用的订阅。"),
    "轻奢": ("👜", "轻奢品消费超出大学生正常消费能力，不建议用生活费购买。经济独立后再考虑品牌消费。"),
    "恋爱": ("💑", "恋爱中过度花费容易造成经济压力，建议坦诚沟通消费观，以平价方式维系感情，避免攀比式约会。"),
    "零食": ("🍟", "零食支出累积惊人！建议每月设定零食预算上限，避免用垃圾食品替代正餐。"),
}

def detect_irrational(row):
    """检测备注和类别中的不良消费关键词，返回额外警示"""
    note = str(row.get("note", ""))
    cat = str(row.get("category", ""))
    amount = row.get("amount", 0)
    warnings_list = []

    for kw, (icon, msg) in IRRATIONAL_KEYWORDS.items():
        if kw in note or kw in cat:
            warnings_list.append(f"{icon} **【不良消费警示】** {msg}")

    # 基于金额和类别的额外检测
    if cat == "娱乐消费" and amount >= 100:
        warnings_list.append("🎮 **【高额娱乐警示】** 单次娱乐消费超过100元，请反思是否为冲动消费。游戏/直播充值应设月度上限，建议不超过总支出5%。")
    if cat == "社交聚餐" and amount >= 200:
        warnings_list.append("🍽️ **【高额聚餐警示】** 单次聚餐消费较高，注意聚餐频率和金额，避免因人情压力超额消费。")
    if cat == "服饰鞋包" and amount >= 500:
        warnings_list.append("👗 **【冲动购物警示】** 单次服饰消费较高，请确认是否为计划内购买而非冲动消费。建议列购物清单后等待24小时再决定。")
    if cat == "美妆护理" and amount >= 300:
        warnings_list.append("💄 **【超额美妆警示】** 单次美妆消费偏高，大学生建议优先基础护肤，避免跟风购买昂贵彩妆。")
    if cat == "人情往来" and amount >= 200:
        warnings_list.append("🧧 **【人情攀比警示】** 人情往来金额较高，量力而行才是真正的成熟。真正的友情不应建立在金钱攀比上。")
    if cat == "基础交通" and amount >= 50:
        warnings_list.append("🚕 **【打车过频警示】** 交通单次消费较高，疑似频繁打车。请多使用公交、地铁等平价出行方式，每次打车可能超出公交10倍费用。")
...
def get_record_advice(row, all_expense_df):
    ...
    if tx_type == "收入":
        if row["category"] == "兼职收入":
            return f"💪 兼职收入 {fmt(amount)}，建议将20%存入备用金账户，剩余合理分配日常支出。"
    ...
    if cat == "餐饮伙食":
        if amount > 80: return f"⚠️ 本次餐饮 {fmt(amount)} 偏高，建议多选食堂，每餐控制在20元左右。当前该类占总支出 {ratio:.1f}%（建议{lo}%～{hi}%）。"
        elif amount <= 25: return f"👍 餐饮 {fmt(amount)} 非常节省！坚持食堂就餐是最省钱的选择。"
        else: return f"✅ 餐饮 {fmt(amount)} 正常。当前占总支出 {ratio:.1f}%（建议{lo}%～{hi}%）。"
    elif cat == "社交聚餐":
        if amount > 150: return f"💡 聚餐 {fmt(amount)} 较高，占总支出 {ratio:.1f}%。建议控制频率，或选择性价比更高的聚餐方式，月度不超过总支出15%。"
        else: return f"😊 聚餐 {fmt(amount)} 适中，维持社交是必要的。注意控制月度频率，当前占比 {ratio:.1f}%。"
    elif cat == "娱乐消费":
        if ratio > 8: return f"🎮 娱乐消费占比已达 {ratio:.1f}%，超出建议上限8%！可利用图书馆、校园免费活动替代付费娱乐。"
        else: return f"🎯 娱乐支出 {fmt(amount)}，当前占比 {ratio:.1f}%，在建议范围内。适度放松有益身心。"
    elif cat == "服饰鞋包":
        if amount > 300: return f"👗 服饰支出 {fmt(amount)} 较大，建议换季统一购买，注意避免冲动消费，优先选择耐用实用款式。"
        else: return f"👕 服饰支出 {fmt(amount)} 合理。注意避免冲动购物，优先选择耐用实用的款式。"
    elif cat == "学习费用":
        return f"📚 学习投入 {fmt(amount)} 非常值得！教育是最好的投资，当前学习支出占比 {ratio:.1f}%（建议{lo}%～{hi}%）。"
    elif cat == "医疗应急":
        return f"🏥 医疗支出 {fmt(amount)}，身体健康最重要！建议平时注意作息，保持锻炼以减少医疗支出。"
    elif cat == "机动备用金":
        return f"💰 备用金 {fmt(amount)}，良好的应急意识！建议始终保持一定备用金余额以应对突发情况。"
    elif cat == "通讯网络":
        return f"📱 通讯网络 {fmt(amount)}，属于必要支出。可对比各运营商套餐，选择性价比最高的方案节省费用。"
    elif cat == "基础交通":
        if amount > 100: return f"🚌 交通支出 {fmt(amount)} 偏高，建议减少打车次数，优先选择公交地铁出行，占比控制在总支出4%以内。"
        else: return f"🚇 交通支出 {fmt(amount)} 合理。坚持公共交通出行是节省的好习惯。"
    elif cat == "美妆护理":
        if ratio > 6: return f"💄 美妆护理占比 {ratio:.1f}%，超出建议上限6%。建议选择平价替代产品，避免盲目跟风消费。"
        else: return f"✨ 美妆护理 {fmt(amount)}，适度即可。优先选择学生党友好的平价产品。"
    elif cat == "人情往来":
        return f"🎁 人情往来 {fmt(amount)}，量力而行是美德。心意比金额重要，注意避免攀比式送礼。"
    elif cat == "生活用品":
        return f"🛒 生活用品 {fmt(amount)}，属于必要支出。可批量采购节省开支，注意避免过度囤货。"
    else:
        if ratio > 5: return f"📋 其他支出占比 {ratio:.1f}%，请关注是否有更合理的分类方式，避免支出模糊化。"
        else: return f"📝 支出 {fmt(amount)} 已记录。建议尽量归入具体分类，便于后续分析。"
...
# ============================================================
# Plotly 布局 — 彻底修复：axis 单独用 update_xaxes/update_yaxes
# ============================================================
def apply_layout(fig, t, title_text="", has_axes=False, grid_color=None):
    """
    所有布局参数通过 **kwargs 展开传入，彻底避免新版 Plotly 嵌套对象校验报错。
    xaxis/yaxis 改用 fig.update_xaxes / fig.update_yaxes 单独设置。
    """
    kwargs = dict(
...
def render_advice(df, settings, theme_name):
        ...
        for _, row in exp_df.iterrows():
            ...
            if warnings_list:
                ...
                for w in warnings_list:
                    st.markdown(f'<div class="advice-card irrational-card">{w}<br><span style="font-size:.78rem;opacity:.6">来源记录：{row["category"]} · {fmt(row["amount"])} · {date_str} · 备注：{row["note"] or "无"}</span></div>', unsafe_allow_html=True)
        if not found_any:
    ...
    if total_assets > 0:
        ...
        elif daily < 60: tips.append("🎯 预算适中，建议餐饮控制在40元/天以内，减少冲动消费。")
...
def render_records_management(df, settings, theme_name):
    ...
    with tab1:
        st.markdown('<div class="guide-tip">💡 每笔收支都会自动更新对应账户余额，并即时生成消费建议。备注中填写具体消费内容（如"奶茶""外卖"等），系统将自动识别不良消费行为。</div>', unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
        ...
        with c2:
            ...
            note = st.text_input("备注（可选，填写消费内容可获得更精准建议）",
                                  placeholder="如：食堂午饭、奶茶、外卖、打车、游戏充值...", key="a_note")
    ...
    with tab2:
        ...
        for _, row in filt.iterrows():
            ...
            if irrational_warnings:
                items = "".join([f'<div style="margin:.2rem 0">⚠️ {w}</div>' for w in irrational_warnings])
                irrational_html = f'<div class="record-irrational">{items}</div>'
...
def render_sidebar(settings):
    with st.sidebar:
        ...
        st.markdown('<div style="text-align:center;font-size:.7rem;opacity:.35;padding:.8rem 0">v6.0 · 本地数据存储<br>Made with ❤️ for Students</div>', unsafe_allow_html=True)
...
def main():
    ...
    if df.empty:
        ...
        ③ 备注中填写具体消费内容（如奶茶、外卖等），系统将自动识别不良消费行为并发出提醒</div>""", unsafe_allow_html=True)
