...
"""
大学生财务管理系统 - Campus Finance Tracker v9.0
新增：密码登录系统，登录后直接加载已有数据
...
import json, os, io, hashlib, hmac
...
def user_data_file(username):
    return f"finance_data_{username}.json"

def user_wallets_file(username):
    return f"finance_wallets_{username}.json"

def user_settings_file(username):
    return f"finance_settings_{username}.json"
...
.login-tab-active{{color:{t['primary']} !important;border-bottom:2px solid {t['primary']} !important;}}
...
.user-chip{{background:{t['primary']}22;border:1px solid {t['primary']}44;border-radius:20px;padding:.3rem .9rem;font-size:.82rem;display:inline-flex;align-items:center;gap:.4rem;color:{t['text']};}}
...
def render_auth_page():
    """渲染登录/注册界面，成功后在 session_state 中设置 logged_in=True, username=..."""
    t = tc("极光紫")
    ...
    with mid:
        ...
        with tab_register:
            ...
            with st.form("register_form", clear_on_submit=True):
                new_user    = st.text_input("用户名（登录用）", placeholder="4-16位字母/数字", key="reg_user")
        ...
        with c2:
            ...
            if not df.empty:
                ...
                st.download_button("⬇️ 下载 CSV", data=buf.getvalue().encode("utf-8-sig"),
                    file_name=f"finance_{username}_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv", use_container_width=True)
...
def render_sidebar(settings):
    ...
    with st.sidebar:
        st.markdown(f"""<div style="text-align:center;padding:1.2rem 0 .8rem">
          <div style="font-size:2.5rem">🎓</div>
          <div style="font-size:1.1rem;font-weight:700;margin:.3rem 0">Campus Finance</div>
          <div style="font-size:.78rem;opacity:.55">v9.0 · 多账号安全版</div>
        </div>""", unsafe_allow_html=True)
