"""
大学生财务管理系统 - Campus Finance Tracker v4.0
===========================================
功能：
1. 钱包初始化（仅记录当前资金，无示例收支）
2. 手动录入收支记录
3. 多维度可视化图表（Plotly 兼容性已修复）
4. 实时资金建议
5. 个性化主题定制
6. 预算监控
7. 数据导出
...
# ============================================================
# Plotly 布局辅助 — 完全兼容新版 Plotly
# ============================================================
def make_layout(t: dict, title_text: str = "") -> dict:
    """
    所有布局参数均使用 plotly 支持的扁平下划线写法，
    但 font 必须传 dict，不能用 font_color / font_family 扁平写法。
    """
    layout = dict(
...
h1,h2,h3 {{ color:{t['text']} !important; font-family:'DM Serif Display','Noto Sans SC',serif !important; }}
...
.wallet-amount {{ font-size:1.4rem; font-weight:700; color:{t['primary']}; }}
.wallet-total {{
...
.empty-state {{
  text-align:center; padding:3rem 1rem; opacity:.55;
}}
...
def render_wallet_panel(theme_name):
    ...
    <div class="wallet-total">
      <div>
        ...
        <div style="font-size:2rem;font-weight:900">{fmt(total)}</div>
      </div>
    ...
    with st.expander("✏️ 修改账户余额", expanded=False):
        st.markdown('<div class="guide-tip">💡 当发生充值/提现/转账等账外变动时，直接在此更新各账户余额。系统会自动记录调整差额。</div>', unsafe_allow_html=True)
...
def render_charts(df, theme_name):
    ...
    grid_color = t["primary"] + "22"

    # ── Tab1 ──
    with tab1:
        if exp_df.empty:
            st.markdown('<div class="empty-state"><div class="empty-icon">📊</div><p>暂无支出数据，记录第一笔支出后图表将自动生成</p></div>', unsafe_allow_html=True)
            return
    ...
    with tab4:
        ...
        layout = make_layout(t, "各星期消费频次")
        layout["xaxis"] = dict(gridcolor="rgba(0,0,0,0)")
        layout["yaxis"] = dict(gridcolor=grid_color)
        fig.update_layout(layout)
        st.plotly_chart(fig, use_container_width=True)
...
def render_advice(df, settings, theme_name):
    ...
    if total_assets > 0 and budget > 0:
        ...
        daily_budget = total_assets / days_left
        ...
        if daily_budget < 30:
            ...
            tips.append("💪 日均预算充足，建议保留 20% 作为应急储备。")
    ...
    if not exp_df.empty:
        ...
        if food > 0:
            days_count = max(len(df["date"].dt.date.unique()), 1)
            avg = food / days_count
            if avg > 80:
...
def render_records_management(df, settings):
    ...
    with tab1:
        st.markdown('<div class="guide-tip">💡 每笔收支都会自动更新对应账户余额。选择正确的支付账户确保数据准确。</div>', unsafe_allow_html=True)
        c1,c2 = st.columns(2)
        with c1:
        ...
        if st.button("💾 保存记录", use_container_width=True):
            ...
            if tx_type == "支出":
                if wallets.get(payment, 0) < amount:
                    st.warning(f"⚠️ {payment} 余额不足（当前 {fmt(wallets.get(payment,0))}），记录已保存，请注意补充余额。")
            ...
            save_data(st.session_state.df)
            st.success(f"✅ 已保存：{tx_type} · {cat} · {fmt(amount)}  →  {payment} 余额更新为：{fmt(wallets[payment])}")
            st.rerun()

    with tab2:
        if df.empty:
            st.markdown('<div class="empty-state"><div class="empty-icon">📝</div><p>还没有任何记录，前往「新增记录」添加第一笔数据吧</p></div>', unsafe_allow_html=True)
            return
...
def render_init_wizard():
    ...
    <div style="max-width:600px;margin:3rem auto;text-align:center">
      ...
      <p style="opacity:.7">首先，请输入您当前各账户的实际余额。<br>这将是系统的起点，后续所有收支都通过手动录入追踪。</p>
    </div>""", unsafe_allow_html=True)

    _, mid, _ = st.columns([1, 2, 1])
    with mid:
        st.markdown('<div class="guide-tip">💡 如实填写当前各账户余额，填 0 表示该账户暂无余额或不使用。后续随时可在「账户余额」模块修改。</div>', unsafe_allow_html=True)
...
def render_sidebar(settings):
    with st.sidebar:
        ...
        if st.button("🏁 重置账户初始化", use_container_width=True, type="secondary"):
            ...
            s2 = load_settings(); s2["wallets_initialized"] = False; save_settings(s2)
            st.rerun()
        ...
        st.markdown('<div style="text-align:center;font-size:.7rem;opacity:.35;padding:.8rem 0">v4.0 · 本地数据存储<br>Made with ❤️ for Students</div>', unsafe_allow_html=True)
...
def main():
    ...
    <div class="welcome-banner">
      ...
      <div style="font-size:.88rem;opacity:.8;margin-top:.4rem">
        {period_label} · {record_count} 条记录 · 总资产 {fmt(total_assets)}
      </div>
    </div>""", unsafe_allow_html=True)
