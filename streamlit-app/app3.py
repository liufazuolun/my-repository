"""
大学生财务管理系统 - Campus Finance Tracker
===========================================
功能模块：
1. 收支记录管理（增删改查）
2. 多维度可视化图表
...
def render_advice(df, settings, theme_name):
    ...
    # 预算执行情况
    st.markdown('<div class="section-header"><span>💡</span><span class="section-title">预算监控</span></div>',
    ...
    # 各类别分析
    st.markdown(
        '<div class="section-header" style="margin-top:1.5rem"><span>📊</span><span class="section-title">消费结构分析</span></div>',
        unsafe_allow_html=True)
    ...
    # 个性化建议
    st.markdown(
        '<div class="section-header" style="margin-top:1.5rem"><span>🎯</span><span class="section-title">个性化建议</span></div>',
        unsafe_allow_html=True)
...
def render_records_management(df, settings, theme_name):
    ...
    with tab1:
        st.markdown(
            '<div class="guide-tip">💡 <b>快速录入</b>：选择收支类型、填写金额和类别，点击保存即可。支持多种支付方式记录。</div>',
            unsafe_allow_html=True)
...
def render_sidebar(settings):
    with st.sidebar:
        ...
        # 时间周期选择（修复核心报错：增加容错处理）
        st.markdown("**📅 数据周期**")
        period_options = ["本月", "本周", "今日", "本学期", "全部", "自定义"]
        # 容错：如果保存的default_period不在选项里，就默认选"本月"
        default_period = settings.get("default_period", "本月")
        if default_period not in period_options:
            default_period = "本月"
        selected_index = period_options.index(default_period)
